...
import { evalRBF, evalRBFGrid2D, type RBFModel } from '../core/rbf';
...
export function stageMetric(model: RBFModel, shape: Shape2D, domain: Box2, boundary?: Float64Array): StageMetric {
  const pts = boundary ?? sampleBoundary(shape, 0.004);
  const m = pts.length / 2;
  let meanAbs = 0;
  for (let k = 0; k < m; k++) meanAbs += Math.abs(evalRBF(model, pts, 2 * k));
  meanAbs /= m;

  const res = 160;
  const grid = evalRBFGrid2D(model, domain.x0, domain.y0, domain.x1, domain.y1, res, res);
